...
export class EncounterResource {
    ...
    async getFixedPenalties(
        @Param('fpnreference') identifier: string,
        @Res() response: Response
    ) {
        try {
    ...
    async getOriginalProhibitionDateTime(
        @Res() response: Response
    ) {
        try {
    ...
    async getOutstandingProhibitions(
        @NotNullQueryParam('identifier') identifier: string,
        @Res() response: Response
    ) {
        try {
            this.logger.addPersistentLogAttributes({identifier});
